...
@section('contenido')
    ...
    <div class="card-header">
        <h3 class="d-inline align-middle">Registrar Combo</h3>
    </div>
    ...
    <div class="card-body" style="margin-top: -20px;">
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-lg-8 col-xs-12">
                    <div class="form-group">
                        <label for="form-label">Nombre del Combo</label>
            ...
            <div class="container border-success pt-3" style="border: 1px solid ">
                <h4 class="d-inline align-middle">Seleccionar Productos Terminados</h4>
                <div class="row pt-2">
                    <div class="col-lg-6 col-xs-12">
                        <div class="form-group">
                            ...
                            <select name="producto" class="selectpicker show-tick form-control form-control-sm" data-live-search="true">
                                <option value="" disabled selected>Seleccionar...</option>
                                @foreach ($p_termins['data'] as $p_termin)
                                <option value="{{ $p_termin['codigo'] }}">{{ $p_termin['descripcion'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
@push('scripts')
    ...
    <script>
        const productos = @json($p_termins['data']);
        ...
        document.querySelector('form').addEventListener('submit', function(event) {
            ...
            fetch('http://localhost/inv_backend/controlador/produccion/agregar_combo.php', {
                method: 'POST',
                headers: {
            ...
            .then(result => {
                if (result.success) {
                    Swal.fire({
                        icon: "success",
                        title: "Combo creado correctamente",
                        showConfirmButton: false,
                        timer: 2000
                    }).then(() => {
                        // Recargar la página después de mostrar el mensaje de éxito
                        window.location.href = "{{ url('combo') }}";
                    });
                } else {
                    Swal.fire({
                        icon: "error",
